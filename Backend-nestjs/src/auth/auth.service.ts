...
export class AuthService {
  private refreshCache = new Map<
    string,
    {
      promise: Promise<{ access_token: string; refresh_token: string }>;
      expiresAt: number;
    }
  >();
  ...
  async oauthLogin(oauthDTO: OAuthDto) {
    ...
    if (user) {
      if (user.deletedAt !== null || user.status === false) {
        throw new UnauthorizedException(
          'Tài khoản của bạn đã bị xoá hoặc khoá. Vui lòng liên hệ cho đội ngũ CSKH để được hỗ trợ',
        );
      }
      const updatedProviders = Array.from(
        new Set([...(user.provider ?? []), provider]),
      );
  ...
  async verifyActivateOtp(verifyActivateOtpDTO: VerifyActivateOtpDto) {
    return await this.usersService.handleVerifyActivateOtp(
      verifyActivateOtpDTO,
    );
  }
  ...
  async sendResetPasswordOtp(
    ...
    return await this.usersService.handleSendForgotPasswordOtp(
      sendForgotPasswordOTPDto,
    );
  }
  async verifyResetOtp(verifyResetPasswordOtpDTO: VerifyResetPasswordOtpDto) {
    return await this.usersService.handleVerifyResetOtp(
      verifyResetPasswordOtpDTO,
    );
  }
    ...
    try {
      ...
      const refreshPromise = (async () => {
        ...
        if (storedToken.revokedAt) {
          ...
          throw new UnauthorizedException(
            'Cảnh báo bảo mật: Refresh token đã bị sử dụng lại. Toàn bộ phiên đăng nhập đã bị hủy.',
          );
        }
        ...
        const { access_token, refresh_token: new_refresh_token } =
          this.generateTokenPair(newPayload);
  ...
  async getMe(userId: string) {
    try {
      ...
      throw new InternalServerErrorException(
        'Có lỗi xảy ra khi get user. Vui lòng thử lại sau',
      );
    }
  }
