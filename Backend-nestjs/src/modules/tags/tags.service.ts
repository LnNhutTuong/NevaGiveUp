...
export class TagsService {
  ...
  async update(id: number, updateTagDto: UpdateTagDto) {
    try {
      ...
      if (
        updateTagDto.status !== undefined &&
        updateTagDto.status !== tag.status &&
        updateTagDto.status === false
      ) {
        if (tag._count.courses > 0 || tag._count.posts > 0) {
          throw new BadRequestException(
            'Không thể thay đổi trạng thái tag này vì đang được sử dụng trong khóa học hoặc bài viết',
          );
        }
      }
      let slug = tag.slug;
      if (updateTagDto.name !== undefined && updateTagDto.name !== tag.name) {
        ...
        const existTags = await this.prisma.tag.findMany({
          where: {
            OR: [{ name: updateTagDto.name }, { slug: slug }],
            id: { not: id },
          },
        });
  ...
  async remove(id: number) {
    try {
      ...
      if (tag._count.courses > 0 || tag._count.posts > 0) {
        throw new BadRequestException(
          'Không thể xóa tag này vì đang được sử dụng trong khóa học hoặc bài viết',
        );
      }
  ...
  async changeStatus(changeStatusDto: ChangeStatusDto) {
    try {
      ...
      if (
        status === false &&
        (tag._count.courses > 0 || tag._count.posts > 0)
      ) {
        throw new BadRequestException(
          'Không thể thay đổi trạng thái tag này vì đang được sử dụng trong khóa học hoặc bài viết',
        );
  ...
  async bulkStatus(bulkStatusDto: BulkStatusDto) {
    try {
      ...
      if (status === false) {
        const tagsInUse = tagsToUpdate.filter(
          (t: any) => t._count.courses > 0 || t._count.posts > 0,
        );
        if (tagsInUse.length > 0) {
          const names = tagsInUse.map((t: any) => t.name).join(', ');
          throw new BadRequestException(
            `Không thể tắt các tag đang được sử dụng: ${names}`,
          );
        }
      }
      ...
      throw new InternalServerErrorException(
        'Có lỗi xảy ra khi cập nhật trạng thái tag',
      );
    }
  }
  ...
  async bulkDelete(bulkDeleteDto: BulkDeleteDto) {
    try {
      ...
      const tagsInUse = tags.filter(
        (tag: any) => tag._count.courses > 0 || tag._count.posts > 0,
      );
      if (tagsInUse.length > 0) {
        const names = tagsInUse.map((t: any) => t.name).join(', ');
        throw new BadRequestException(
          `Không thể xóa các tag đang được sử dụng: ${names}`,
        );
      }
